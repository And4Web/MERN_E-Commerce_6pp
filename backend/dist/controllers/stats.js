...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has(key))
        ...
        const latestTransactionsPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthOrders, thisMonthProducts, thisMonthUsers, lastMonthOrders, lastMonthProducts, lastMonthUsers, productsCount, allOrders, usersCount, lastSixMonthsOrders, categories, femaleUsersCount, maleUsersCount, latestTransactions,] = await Promise.all([
...
export const getPieChartStats = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has(key))
        ...
        const [processingOrders, shippedOrders, deliveredOrders, categories, productsCount, productsOutOfStock, allOrders, allUsersDOB, allAdminUsers, allCustomerUsers,] = await Promise.all([
...
export const getBarChartStats = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has(key))
        ...
        const [sixMonthsProducts, sixMonthsUsers, twelveMonthsOrders] = await Promise.all([
        ...
        const productsCount = getChartData({
            length: 6,
            today,
            documentArray: sixMonthsProducts
        });
        const usersCount = getChartData({
            length: 6,
            today,
            documentArray: sixMonthsUsers
        });
