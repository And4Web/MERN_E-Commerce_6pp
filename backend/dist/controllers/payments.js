...
export const createPaymentIntent = TryCatch(async (req, res, next) => {
    ...
    // reminder: remove paymentIntent from response, only send clientSecret for frontend purpose.
    return res.status(201).json({ success: true, message: "New payment intent created", clientSecret: paymentIntent.client_secret, paymentIntent });
});
// route - /api/v1/payments/coupon/new
export const newCoupon = TryCatch(async (req, res, next) => {
    const { couponCode, amount } = req.body;
    if (!couponCode || !amount)
        return next(new ErrorHandler("Enter both coupon code and amount", 404));
    await Coupon.create({
        couponCode, amount
    });
...
export const allCoupons = TryCatch(async (req, res, next) => {
    ...
    let message = "";
    if (!allCoupons)
        message = "No active coupons at this moment.";
    else
        message = `${allCoupons.length} active coupons at this moment.`;
...
export const deleteCoupon = TryCatch(async (req, res, next) => {
    ...
    if (!coupon)
        return next(new ErrorHandler("No coupon with such Id found.", 404));
